 ...
 * for a given segment.
 *
 * @param _dir                                日志所在的文件夹路径
 * @param config                              The log configuration settings
 * @param segments                            The non-empty log segments recovered from disk
 * @param logStartOffset                      日志的当前最早位移.The earliest offset allowed to be exposed to kafka client.
...
object Log extends Logging {
  ...
  /**
   * 通过给定的位移值计算出对应的日志段文件名, 其实就是补齐20位
   *
  ...
  def filenamePrefixFromOffset(offset: Long): String = {
    ...
    nf.setMinimumIntegerDigits(20) //设置数字的整数部分允许的最小位数20
    nf.setMaximumFractionDigits(0) //设置数字的小数部分允许的最大位数0
